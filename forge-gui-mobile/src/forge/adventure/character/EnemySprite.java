...
//            }
    ...
    public Array<Reward> getRewards() {
        ...
        // Round 289: the collection this payout is built into. Upstream's 09.22 refactor replaced the
        // method's old `Array<Reward> rewards = new Array<>()` with a reused per-enemy pool, and the merge
        // took that rewrite for the chaos-battle section above (stock code we never touched) - which left
        // the TFR pipeline below with nothing to add to. A fresh Array is deliberate here rather than the
        // pool: getReward() hands this straight to appendCoinRansomReward(), which APPENDS to it, and the
        // RewardScene holds it while it draws. The pool is cleared at the top of every getRewards() call, so
        // sharing it would let a second call empty an array the scene is still showing. One allocation per
        // duel win is not what upstream's pooling was aimed at - that was the per-frame paths.
