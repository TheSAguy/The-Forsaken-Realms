...
## STATE 2026-09-22 (round 285; v1.12 "Reward Balancing" RELEASED, rounds 247-285 after it, UNPUSHED; ENGINE = 09.18 daily since round 242) - READ THIS FIRST, DO NOT REPEAT WORK

- **NEXT SESSION starts here (updated round 285, 2026-09-22).** v1.12 is out; rounds 247-285 are unreleased. In order:
- **DO NOT `git push` until the user calls a release (their rule, 2026-09-20: "Let's not update the online repo
  ...
  1. **Round 285 - the Armory's buttons move to the bottom; 1.13 CONFIRMED RUNNING ON ANDROID.**
     ...
     the card grid. New `layoutPortraitModButtons()` uses ArenaScene's bottom-row shape (round 263):
     a block just above Done, spanning the GRID's own content width, two per row, **an odd last button
     takes the full width** so there is no hole; at most 4 are ever visible together. **Where the space
     comes from is the design decision:** NOT `ui/items_portrait.json` (shared by every plane -
     Shandalar's shops show none of these buttons and would lose height for nothing) but at RUNTIME,
     inserted between the `case Shop:` visibility switch (~line 888) and the card-fitting loop
     (~1035), which is the one point where the buttons are known and `targetHeight`/`targetArea` are
     not yet consumed - one pass, no re-layout, and the cards SCALE (they are fitted by an area search
     over the container, not placed at fixed size). 85 units for the 3-button case; grid keeps 3x3.
     LANDSCAPE UNTOUCHED. Mocked at 270x480 with real labels BEFORE building - which caught the first
     attempt clipping `playerGold` by 8 units. **Android 1.13 verified on the emulator**: the missing
     piece was never a path but `MANAGE_EXTERNAL_STORAGE`, which the APK declares and which had been
     REJECTED - without it the app cannot read `/sdcard/Download` or `/sdcard/Pictures` (the user's
     shared-folder copy was there and unreadable) and scoped storage even hides a `shell`-owned file in
     the app's OWN obb dir; `appops set com.thesaguy.forsakenrealms MANAGE_EXTERNAL_STORAGE allow`.
     Also: **LDPlayer 14's ADB bridge is off by default** (`basicSettings.adbDebug` 0 in
     `vms/config/leidian0.config`, only port 2222 listening; flip it with the instance STOPPED or
  ...
  2. **Round 284 - a Basilica guard, and an APK that can be handed its own assets.** User: *"add a guard
     ...
     chest (14 px / 28 px against the 48 px reach), reachable per the audit, STATIONARY with
     `threatRange` 20 like the Angel already guarding this map's booster. Only two white Phyrexian enemies
     exist and both are Adept, so the no-Apprentice rule holds. Checked: `enemy.tx` defaults all four
     `spawn.*` to true, which matters because `canSpawn()` unboxes all four and the user plays HARD.
     **Also fixed here:** enemy 74's route was `81,82` = tiles (19,1)/(18,1), the sealed strip right above
     those chests, so it left the map in this very corner - while waypoints 84 tile(20,4) and 85 tile(19,5)
     sat correctly in the corner referenced by NOTHING. Rewired 74 to 84,85; round 283's audit calls the map
     clean. (The 81-85 cluster means the author DID intend a guard here - it never worked for exactly the
     round-279 reason, so a stationary guard is the fix, not a better route.) **`AssetsDownloader` now prefers
     a local assets.zip** over downloading: user asked for the APK to read
     `C:\Users\User\Pictures\Screenshots\Android`, which is impossible (it runs INSIDE the emulator; a
     Windows path does not exist there) but the diagnosis was right - an unreleased build 404s on
     `tfr-v1.13/assets.zip`, a SEPARATE issue from round 282's updater fix. It searches the app asset dir,
     `/sdcard/Download|Pictures|Documents|Screenshots` and `/sdcard`, **logging every path checked** so a miss
  ...
  3. **Round 283 - nobody had ever validated a patrol ROUTE, and a capital W is protecting the game.**
     ...
     `magetower_13_doppelganger.tmx` (one of SIX maps display-named "Blue Tower"; identified by its shape,
     since the log was locked by the running game): `treasure 101` at tile(11,15) is guarded by `enemy 74` at
     (14,15), route `106,107`. Row 14 is the room's NORTHERNMOST walkable row and waypoint 107 sat at
     tile(11,13) - one tile past it, in drawn-but-sealed void that carries NO collision, so the walk out was a
     perfectly executable instruction. Moved to tile(14,14): reachable, north of both chest and guard, a
     three-tile pace. Only enemy 74 references it and `common/`'s copy lacks object 107 entirely.
     **New `dev-tools/waypoint_routes_qa.py`** - routes hold OBJECT IDS of `waypoint.tx` objects, and
     `map_objects()` keeps only entry/enemy, which is why they were invisible. **Its first run cried wolf**
     (16 false "dangling"): the grammar is `waitN` = pause, `rA-B-C` = pick one at RANDOM, `wN` = pause, bare
  ...
  4. **Round 282 - the 1.13 APK offered itself a DOWNGRADE on every launch, and closing was stock doing
  ...
  5. **Round 281 - the softlock watchdog was crying wolf; 1.13 is stamped for an emulator test.** Round 279's
     ...
     stage, and a `TypingLabel` that never acts never ends, so their buttons stayed hidden - correctly, they were
     not on screen. Fix: a button counts only while `getStage() != null`. **`hasParent()` would NOT work** - in
     the scrolling layout a detached button keeps its equally-detached `optionHost` as a parent, so every
     >6-option dialog would still cry wolf. The watchdog also stopped waiting out the deadline: two signals polled
     twice a second - `hasEnded()` still true after the reveal stagger (window derived from the button count), and
     no new character for 2s via `TypingListener.onChar()` - with the scaled deadline kept as the backstop. A real
     freeze releases in ~2s instead of 23. Safe because **no plane's dialog text uses `{WAIT}` or a speed token**
     (checked all seven) and because **`onChar` is really invoked** (verified in textratypist 0.8.2's bytecode - if
     it were not, every dialog in the game would reveal its options 2.5s in). **The grey window is therefore still
     unexplained and still one report**; the candidate line was this false positive. The next report answers
     itself - a watchdog line means the stall is real, a `dialog N shown` line with none after it means something
     else. **1.13 stamps** per ANDROID_RELEASE.md 1-2: `modVersion` 1.13 + `modVersionDate` 09.21, `tfr.version`
  ...
  6. **Round 280 - rob a guard and it hunts you.** User: *"If the Booster or Chest is taken, and the guard is
     ...
     included** (`EnemySprite.guardedRewardId`), but the behaviours differ on purpose: holding a post stays
     BOOSTER-only (round 279's ask), chasing the thief covers both. `enrageOverStolenLoot()` drops the post, sets
     `aggro` directly (the theft is the trigger - the player may already be at the door), clears `fleeRange`, and
     raises threat/pursue to the map's diagonal so pursuit cannot lapse in-room. A dead guard is not in `actors`,
     so "still alive" is free. **THE JUDGMENT CALL:** the median enemy runs speed 30 against the player's 40 and
     only 434 of 1,974 are faster than the player, so without a speed bump the chase could never close for ~4/5
     of guards - it would be scenery. `robbedGuardSpeedFactor` x `playerBaseSpeed`, derived not hardcoded (the
  ...
  7. **Round 279 - 85.6% of the plane's enemies could notice the player and never chase, and round 271 caused
     ...
     only ever reached enemies with NO threatRange. Measured: **2,021 of 2,361 placements (85.6%)** were in that
     state, against 148 healthy. Round 271 stamped `threatRange=20` into 251 enemies that had none, moving them
     from "gets both defaults" into "cannot chase" - its stated goal was the opposite. Ungated now, using the
     method's own `max(threatRange, mapEnemyDefaultPursueRange)` = round 252's 64 px. **BLAST RADIUS: 2,021
     enemies across every hand-authored map now chase for ~4 tiles where they chased for none.** Intended, but
     it is a real difficulty change and NOT YET SEEN IN A GAME.
     **Booster guards now hold their post:** `EnemySprite.guardPost` walks a guard back on losing aggro and
     holds it there, placed BEFORE the `movementBehaviors` deque so a patrol route cannot walk it off the
     booster - which is what the Blue Tower screenshot actually showed (its boosters have guards within one
     tile, and five of that map's seven enemies carry `waypoints`). `MapStage.assignBoosterGuards()` pairs each
     booster with its nearest non-dialog enemy within 3 tiles at load; `[TFR-BoosterGuard]` logs it per map.
     **Coverage: 261 boosters, was 71 unguarded -> 2.** The 2 left are `cave_spider.tmx` (guard present, no
     entry object to verify reachability from) and `phyrexian_black1.tmx` (round 278's misplaced-entry map).
     **49 Apprentice guards promoted to Adept+** by `questTags` overlap on CREATURE tags only - counting
  ...
  8. **Round 279 - the reported "grey window" is a `MapDialog` with its buttons still invisible.** A player
  ...
  9. **Round 279 - `validate_plane_data.py` had been crying wolf for eighteen rounds.** It reported round 261's
  ...
  10. **Round 278 - fifteen dungeons could never be cleared, and round 275's "leave them alone" was wrong.**
     ...
     The discriminator is not the creature's name but **could the PLAYER stand on that tile if a route
     existed**: `dev-tools/stranded_enemies.py` splits round 275's 52 into **28 STRANDED** (legal player tile,
     no route) and 23 standing where collision covers the tile (water, chasm, roof - where a swimmer or flier
     belongs and round 252's reaction range brings it out to the player). The 28 are a real bug:
     `AdventureQuestController.updateQuestsWin()` sets `allEnemiesCleared` false if ANY enemy remains except
  ...
  11. **Round 277 - a player standing inside a POI footprint could not path anywhere.** Found by soaking: after
  ...
  12. **Round 276 - the agent bridge left a zombie process, and `back` during a duel quit the game.** Both found
     ...
     bridge already sets) which inherits daemon from its caller - the render thread. It answered `/state` with an
     all-null snapshot for eight minutes, which reads as a freeze, while holding the saves and blocking a
     package. `server.start()` now runs on a daemon thread. Separately, a `DuelScene` is a `ForgeScene`, so
  ...
  13. **Round 275 - reachability exists, and round 269's audit was reading a mirror.** THE COLLISION QUESTION IS
     ...
     feet** (`(x+4, y, width-6, height*0.4)`), not the 16x16 sprite. That is why legitimate enemies stand on
     "collision tiles". Walkability is a POSITION question, never a tile question.
     `dev-tools/pixel_collision_qa.py` already did all of this; a second copy written this round was DELETED, and
     that file gained entry seeding, template-resolved objects and an `--enemies` report instead.
     **Round 269's audit used a .tmx tile object's y as a bottom-up tile index, which is a vertical MIRROR** - a
     tile object's y is its BOTTOM edge in a top-down grid. Measured against 82 entry objects (provably standable,
     the game spawns onto them): the correct conversion puts 97.6% on legal ground, round 269's 8.5%. So the "13
     pre-existing out-of-bounds enemies" were an artifact; 12 of them are perfectly reachable. **One was real** -
     the Mimic in `library_of_varsil_3.tmx` at (512,-32) on a 480x320 map - and is moved and verified.
     **The ten round-269 guards are re-placed** by `dev-tools/guard_placement.py`, each one tile from its booster,
     each verified reachable (candidates are whole-tile offsets from the booster's OWN authored coordinates, so no
     conversion is involved in a write). **51 other "unreachable" placements are deliberately left alone**: they
     are Jellyfish, Crocodiles, Griffins, Bats and 9 Farmers - things that swim, fly or stand in crops, which
     round 252's reaction range brings to the player. **For the user to look at:**
  ...
  14. **Round 274 - `innTournamentQuestGiven` is seeded at load** from `statistic.completedEventCount() > 0` when
  ...
  15. **Round 273 - the importer no longer assumes the art faces right.** The PORTRAIT half is fixed outright and
  ...
  16. **Round 272 - the claim-path index-space trace, and round 266's assumption was half wrong.** `terrainMap`
  ...
  17. **Round 271 - all 251 rangeless enemies now react.** `threatRange=20` written into the maps (not left to
     round 252's runtime default). 23 `dialog` NPCs and the explicit `0`s deliberately untouched. **PACKAGED.**
  18. **Round 270 - castles show the unvisited magnifier.** `MapSprite`'s rule listed the two GENERATED types
  ...
  19. **Round 269 - ten of round 258's guards stood outside the room.** CLOSED by round 275 (see the round 275 item): all
  ...
  20. **DONE in round 272 - the trace that was queued here.** Round 266's assumption was half wrong and round
     ...
     reaches the claimed-wasteland decode. See the round 272 item above and MOD_CHANGELOG round 272.
  ...
  3. **Round 267 - speed, silence, and a stop script that works.** `playerBaseSpeed` 40 -> 44 (walking only;
     the day clock is untouched). The Capitol surge no longer announces itself - arrivals still do.
     **`agent_stop.ps1` matched `*TFR-Agent*` and so could never find the agent after the C: move** - it now
     matches the agent's folder and refuses anything under `C:\TFR\live`. Verified with both games running.
  4. **Round 266 - the rim specks: one rule for drawing a minimap tile.** The full re-bake was a near-copy of
     `redrawMinimapTile()` that never learned round 257's claimed-wasteland rule, so it decoded expansion-claimed
     tiles against the claiming colour's tables. Both paths are now `drawMinimapTile(target, x, y, decodeBiome)`,
     with the re-bake deriving the decode biome from `isClaimedWasteland()`. **No before/after image** - nothing
     triggered a re-bake during the agent run; the argument is structural.
     **Open question for the user:** their save-3 screenshot shows a tan brick-textured network tracing the
     green patches, which reads as the ROAD tileset (the star roads plus the ring roads), not the rim artifact.
     Ask what specifically looks wrong there before treating it as the same bug.
  5. **Round 265 - a repelled attack leaves the town alone.** A lost capture roll used to revert an enemy
     colour's town to neutral, so attacking could not lose: take it on a win, deny it on a loss. Now the mage is
     spent and the defender holds. **User's call, cost accepted:** colours consolidate, fewer towns drift back
     to neutral, Ring Cities stay held longer (3-of-5 loss pressure). Sacking (20%, on a WON attack) is the
     remaining route to a neutral ruin. The revert path and round 263's rewording of it are gone with it.
  6. **Round 264 - the REAL cause of the huge Archmages.** Round 178 made `EnemyData.scale` the normalizing
     factor and removed the `scale = 1.0f` override from `dispatch()`; its companion in `WorldStage`'s
     roaming-enemy LOAD path survived, so every territory mage was un-normalized by 1/scale on every save/load.
     16px wizard (1.08) looked fine, a 124px dragon (0.30) drew 3.3x too big - exactly the user's per-colour
     report. Round 261's ceiling was masking this, not curing it; it stays as a backstop.
  7. **Round 263 - the Arena fits a phone; a freed town names its attacker.** Round 216's three-button row was
  ...
  8. **Round 262 - editions with no cards are gone from the research station.** `Unlimited Edition (2ED) (0/5) -
  ...
  9. **Round 261 - a ceiling on what a roaming enemy draws.** The surge's Archmages filled the screen. Cause was
  ...
  10. **Round 260 - the Capitol surge targets correctly.** Round 255 searched every attackable town for the one
  ...
  11. **Round 259 - "Participate in an Inn Tournament" is once per player.** Round 245's gate reads
  ...
  12. **Round 256 - the obstacles inside a town's icon, frozen set-pieces, half the rares.** The clear-ground pass
  ...
  13. **Round 255 - the opening reads as one story, plus four corrections from play.** Quest 53 gained **stage 3
  ...
  14. **Round 254 - the five things a new game found in round 253.** Entry boxes are capped at two tiles
     ...
     cave once round 253 made the spawn POI inactive - their log caught it); the **Capitol POI carries the
     `Orazca` tag** so quest 43 and quest 52 still resolve after the upgrade renames it; the **Warden is in
     `player_capital.tmx`** (he was missing entirely - the upgrade swaps the map) and left of the board building
  ...
  15. **Orazca, the ruin at the centre of the star (round 253).** The campfire's tile is now a ruined town named
     ...
     Orazca)". The spawn cave keeps its campfire ~5 tiles aside and still opens the game; the Warden keeps only the
     introduction there and deletes himself when it ends, and now stands inside Orazca with the rest of his
     ...
     **SEEN IN A RUNNING GAME** (agent game, a brand-new world): out of the cave at tile (353, 355), Orazca 3 tiles
     ...
     and out of the road pass - while still opening every new game); the Warden stands beside Orazca's bulletin
     board; **one Jumpstart tournament per PLAYER** (`jumpstartPlayed` now survives the New Game+ flag wipe - it was
     once per run, so every NG+ handed out another); and an **Android** fix - the shop card grid could overflow its
     region and swallow the taps meant for Upgrade Armory / Expand Inventory / Storage (`[TFR-RewardGrid]`), which is
     what the player report about "buying the items in the way" was. NOT confirmed on a phone yet.
     **`MapDialog` note worth keeping:** `activate()` loads EVERY entry whose condition passes, each overwriting the
     ...
     first.
  16. **Enemies that never reacted (round 252).** 378 of the plane's 2,359 map enemies had no `threatRange` and no
  ...
  17. **The map (rounds 249-251), all three SEEN IN A RUNNING GAME (round 251's entry).** 249: every icon centered on
  ...
  18. **Rounds 247-251 are IN THE F: LIVE FOLDER** (packaged 15:04, jar `be6bb7ea998b`; C: and the agent folder match).
  ...
  19. **Playtest v1.12.** Seen in a running game: the card budget, the resource purse, the pickup labels, the payday
...
- Round 267 (2026-09-20): **walking speed +10%, the Capitol surge goes quiet, and `agent_stop.ps1` can find the
  agent again** - it matched a path that stopped existing when everything moved to C:, so it always reported
  nothing running.
- Round 266 (2026-09-20): **the rim specks.** The full minimap re-bake was a second copy of the per-tile draw
  that never learned round 257's claimed-wasteland decode. One function now, with the decode biome derived.
- Round 265 (2026-09-20): **a failed assault no longer costs the defender the town.** MOD_SCOPE #7's revert was
  deliberate but made attacking risk-free; a lost roll is now a repel. User's call, with the consolidation cost
  accepted. The revert path is removed.
- Round 264 (2026-09-20): **the huge Archmages, actually fixed.** `WorldStage`'s load path still forced
  `scale = 1.0f` on territory mages - the companion round 178 missed - so every save/load un-normalized them by
  1/scale. Round 261's ceiling was hiding it.
...
- Round 256 (2026-09-20): **the obstacles standing inside a town's icon.** The sweep ran before the biomes'
  structures and the roads were stamped, so it cleared nothing that lasted (the log's honest "0 removed" while a
  boulder sat against Orazca). It runs last now, sized to each POI's icon, zeroing only collision/structure cells,
  and once per existing save on load. Plus: the dueling club, Valor's Reach Arena, the Naktamun gym and debug_map
  are frozen again (39 enemies, `threatRange -1`), and the seven town quests pay 2 rares instead of 4.
...
- Round 254 (2026-09-20): **the five things a new game found in round 253**, plus the Capitol's answering Archmage.
  ...
  `allowInactivePOI` (round 253's inactive POI had sent it to a random cave); the `Orazca` tag added to the Capitol
  POI so Orazca-targeted stages survive the upgrade; the Warden added to `player_capital.tmx` and moved left of the
  board building in both towns; `dispatch()` gained `forceArchmage`/`furthestTarget`, used by the Capitol surge.
- Round 253 (2026-09-19): **Orazca, the ruin at the centre of the star.** The campfire's tile became a ruined town
  the player restores first and crowns later - the only seat the Capitol can rise from (same 5-town gate, same cost),
  never seeded as a functioning neutral town, never renamed from the waste pool, never attackable until it is the
  Capitol; the star's spokes radiate from it. The spawn cave steps ~5 tiles aside and keeps the opening; the Warden
  keeps only the introduction there and moves into the town for everything after it (which also un-shadowed the
  five-keys trade). `findHome()` = Capitol, else Orazca, else the cave, for the Homeward rune and death respawn. New
  worlds only. Also: round 252's default reaction radius now skips the 43 dialog NPCs it would have set walking; no
  campfire at all (the spawn POI is inactive); one Jumpstart tournament per PLAYER, not per run (`jumpstartPlayed`
  survives NG+); and the Android shop grid no longer covers its own buttons.
...
- Round 240 (2026-09-18; rounds 238 + 239 + 240 built together 21:50, PACKAGED 22:03 - 346 MB, `PACKAGER EXIT 0`, live
  ...
  **quest 92 "Sweep the Wilds"** - clear three dungeons/caves (new objective `ClearDungeons`,
  ...
  still overworld kills only. **Quest log shows progress** - `AdventureQuestStage.getProgressText()` " (2/5)" + a HUD
  line per step. quests.json round-trips byte-identical through json.dumps(indent=tab). FLAGGED to the user: quest
  REWARDS are outside the card budget and some are lavish (Slobad's quest: 10 rare/mythic artifacts, 14 below Insane).
...
- Round 193 (2026-09-13, PACKAGED with 195): **the claim loop short-circuits.**
  ...
  minimums for a decision that only asks "did anything beat me?", so it now stops at the first source that
  settles the tile. Exact (every exit reaches the same `continue`; the minimums were read nowhere else).
  **Verified with `dev-tools/ClaimEquiv.java`: 160,000 randomized cases, 0 mismatches, identical claim counts.**
  Needs a real log to confirm the ms saving.
...
- Round 180 (2026-09-11, built 13:06, PACKAGED 13:18, 342 MB; agent folder synced): **monster loot follows its color's sets for every roamer** - `EnemySprite.getRewards()`
  ...
  The **bestiary page** of the 197: https://claude.ai/code/artifact/5f82734c-8bf7-497f-a369-27a1934c221e. Round-179 corrections: mix 64 / 66 / 67; the chest pools skip the new ones.
- Round 179 (2026-09-11, data only, PACKAGED 12:21, 342 MB; agent folder synced): **197 new enemies** from the user's art - 124 Ragnarok Online + 73
  render sheets, MTG names; 48 Apprentice / 73 Adept / 51 Master / 25 Archmage; 64 mono / 66 two / 67 three-color; roaming
