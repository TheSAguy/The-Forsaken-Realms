...
- **`forge-gui-mobile/src/forge/adventure/stage/MapStage.java`** — round 283 changed NOTHING here, and round
  ...
  tested, and `dev-tools/waypoint_routes_qa.py` shows `templeofchandra`'s Magma Elemental route alone carrying
  ~23 legs that are blocked or outside the playable area. The typo is currently the only thing keeping those
  routes dormant. Repair the routes, THEN flip the case, then play the story maps — in that order.
...
`util/AdventureQuestController.java` - `showQuestDialogs()` holds quest 74 in a ruined town.
`util/TownRestoration.java` - `isCurrentTownRuined()`, `logInnQuestHeldInRuin()`. `scene/EventScene.java` -
`INN_TOURNAMENT_TUTORIAL_QUEST_ID` is public. Five `sprites/enemy/tfr/*.png` Avatar cells.
...
`ClearDungeons`, new `updateDungeonCleared()`. `data/AdventureQuestStage.java` - `handleEvent()` case
`ClearDungeons`, progress HUD line on `Defeat`, new `getProgressText()` / `announceProgress()`.
`util/DungeonRotation.java` - `onDungeonClear()` fires the event on a real despawn. `scene/QuestLogScene.java` -
progress text on active stages. `world/quests.json` - quest 92 added, quests 54-63 raised to five kills.
...
`forge-gui-mobile/src/forge/adventure/character/EnemySprite.java` - `getRewards()` generates the type's standard
rewards into their own array, passes them through `CardBudget.apply()` (with the enemy's deck and the color's
edition restriction), then adds the map-instance rewards untouched.

`forge-gui-mobile/src/forge/adventure/data/TuningData.java` - fourteen `cardBudget*` fields.

`forge-gui/res/adventure/The Forsaken Realms/config tables/settings.json` - the same fourteen keys.
`dev-tools/validate_plane_data.py` - the keys added to the TuningData field list.
...
against new private `retroFlagThreshold()`; new public `reopenIfCompletedByUnsetFlag()` and
`deactivateIfPrerequisiteOpen()`.

`forge-gui-mobile/src/forge/adventure/data/AdventureQuestData.java` - new public
`reopenStagesCompletedByUnsetFlag()`.

`forge-gui-mobile/src/forge/adventure/player/AdventurePlayer.java` - `load()` runs it over the loaded quests.

`forge-gui/res/adventure/The Forsaken Realms/world/quests.json` - `"mapFlagValue": 1` on quest 52's five Rescue
stages.
...
`drawTeleporterIndicator()` after `drawGuardIndicator()`.

`forge-gui-mobile/src/forge/adventure/util/EconomyBuildings.java` - new public static `getTeleporterMapIcon()` and
its one-second network-active cache.
...
`bonusDeckCards()` for it). No plane data changed.
...
`adventure/data/TuningData`, and plane data (`config tables/settings.json`, `world/quests.json`).
...
their stone cost into wood; LUMBER_MILL keeps the old all-stone price and no longer shares the line).
...
`[TFR-RewardDup]`), `adventure/character/EnemySprite` (opens one payout scope around its reward
loops), `adventure/scene/InventoryScene` (sell button hidden).
